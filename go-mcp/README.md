...
[MCP](https://modelcontextprotocol.io) server exposing the KoreanJson SDK as
two agent tools — `korean-json_list` and `korean-json_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/korean-json-mcp)
make build

# 2. Provide credentials via the environment
export KOREAN_JSON_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user korean-json \
  -- /absolute/path/to/korean-json-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./korean-json-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// korean-json_list: first page of records
{ "entity": "comment" }
{ "entity": "comment", "query": { } }

// korean-json_load: one record by id
{ "entity": "comment", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/korean-json-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user korean-json \
     -- "$PWD"/dist/*/korean-json-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `korean-json_list` and `korean-json_load` tools now appear
   in new sessions. Ask the agent to *"list comment using korean-json"*
   and it calls `korean-json_list` with `{"entity":"comment"}`.
...
```sh
export KOREAN_JSON_APIKEY=sk_live_xxx            # API key
export KOREAN_JSON_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `korean-json_list` tool
...
### Call the `korean-json_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `korean-json_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `korean-json_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `KOREAN_JSON_APIKEY` | API key sent with every request. |
| `KOREAN_JSON_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

comment | post | todo | user

### Smoke test via HTTP (raw JSON-RPC)

```sh
./korean-json-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"korean-json_load","arguments":{"entity":"comment","query":{"id":1}}}}'
```

...
The Golang SDK for the KoreanJson API. Provides an entity-oriented interface using standard Go conventions — no generics required, data flows as `map[string]any`.


## Install
```bash
go get github.com/voxgig-sdk/korean-json-sdk
```
...
```bash
go mod edit -replace github.com/voxgig-sdk/korean-json-sdk=../path/to/github.com/voxgig-sdk/korean-json-sdk
```
...
```go
// Create
created, _ := client.Comment(nil).Create(
...
// Update
client.Comment(nil).Update(
    map[string]any{"id": newID, "name": "Example-Renamed"}, nil,
)

// Remove
client.Comment(nil).Remove(
...
| `User` | `(data map[string]any) KoreanJsonEntity` | Create a User entity instance. |
...
```
github.com/voxgig-sdk/korean-json-sdk/
...
The root package (`github.com/voxgig-sdk/korean-json-sdk`) re-exports everything needed

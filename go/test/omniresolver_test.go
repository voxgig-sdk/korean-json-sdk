...
import (
	...
	omni "github.com/voxgig-sdk/korean-json-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.KoreanJsonError); is {
				out := map[string]any{
					"name":    "KoreanJsonError",
...
func omniCtx(arg any, client *sdk.KoreanJsonSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.KoreanJsonSDK); is {
			client = live

...
func (sdk *KoreanJsonSDK) OptionsMap() map[string]any {
...
func (sdk *KoreanJsonSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *KoreanJsonSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *KoreanJsonSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *KoreanJsonSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *KoreanJsonSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *KoreanJsonSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("KoreanJsonSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *KoreanJsonSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *KoreanJsonSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("KoreanJsonSDK: graphql: %s", msg)
		res["graphql"] = errors
	}

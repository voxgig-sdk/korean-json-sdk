...
Learn more about Korean JSON API at [koreanjson.com](https://koreanjson.com).

This is an unofficial SDK for the Korean JSON public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Comment, Post, Todo and User — that you
...
const items = await client.Comment().list()
```
...
// List all comments (returns CommentEntity[] — .data() for the record)
...
| **Post** | The Post entity (create, list, load, remove, update). | `/posts` |
| **Todo** | The Todo entity (create, list, load, remove, update). | `/todos` |
...
$comments = $client->Comment()->list();
print_r(array_map(fn($item) => $item->data_get(), $comments));

// Load a specific comment (returns the ENTITY; call data_get() for the record; throws on error)
$comment = $client->Comment()->load(["id" => 1]);
...
# Load a specific comment (returns the ENTITY; call data_get for the record)
comment = client.Comment.load({ "id" => 1 })
puts comment
```
...
- Upstream API: [https://koreanjson.com/](https://koreanjson.com/)

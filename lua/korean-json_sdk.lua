...
local KoreanJsonSDK = {}
KoreanJsonSDK.__index = KoreanJsonSDK
...
KoreanJsonSDK._make_feature = _make_feature


function KoreanJsonSDK.new(options)
  local self = setmetatable({}, KoreanJsonSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function KoreanJsonSDK:options_map()
...
function KoreanJsonSDK:get_utility()
  return Utility.copy(self._utility)
end


function KoreanJsonSDK:get_root_ctx()
  return self._rootctx
end


function KoreanJsonSDK:prepare(fetchargs)
...
function KoreanJsonSDK:direct(fetchargs)
...
-- by passing it.
function KoreanJsonSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function KoreanJsonSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "KoreanJsonSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function KoreanJsonSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = KoreanJsonSDK.new(sdkopts)
  sdk.mode = "test"

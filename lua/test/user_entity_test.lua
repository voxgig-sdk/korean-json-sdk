...
describe("UserEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set KOREAN_JSON_TEST_USER_ENTID JSON to run live")
      return
    end
    ...
    local user_ref01_markdef_up0_name = "city"
...
function user_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("KOREAN_JSON_TEST_USER_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["KOREAN_JSON_TEST_USER_ENTID"])
  if idmap_resolved == nil then
    idmap_resolved = helpers.to_map(idmap)
  end

  if env["KOREAN_JSON_TEST_LIVE"] == "TRUE" then

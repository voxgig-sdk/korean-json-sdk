...
boru-driven command-line client **and** interactive REPL for the KoreanJson
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/korean-json-cli)
make build

# 2. See usage (words, entities, env vars)
./korean-json-cli --help
...
./korean-json-cli load 1 comment            # {id:1} shorthand
./korean-json-cli load '{id:1}' comment       # explicit match map
./korean-json-cli update '{name:"x"}' comment
./korean-json-cli list post

# 5. Override the API base URL for a single call
KOREAN_JSON_BASE=https://api.example.com ./korean-json-cli list comment

# 6. No arguments -> interactive REPL
./korean-json-cli
korean-json> list comment
korean-json> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/korean-json-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/korean-json-cli list comment
   ```
...
```sh
./korean-json-cli load 1 comment          # scalar shorthand for {id:1}
./korean-json-cli load '{id:1}' comment     # explicit match map
```
...
```sh
./korean-json-cli update '{id:1,name:"new"}' comment
```
...
Run with no arguments to open a REPL (prompt `korean-json>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `comment`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/korean-json-cli`. |
...
(`./korean-json-cli <expr>`) and interactively (the REPL), and expressions compose the

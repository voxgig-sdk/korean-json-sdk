// Command KoreanJson-cli is a CLI + REPL for the KoreanJson SDK,
...
const prompt = "korean-json"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "comment post todo user"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      KOREAN_JSON_APIKEY (api key), KOREAN_JSON_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: KOREAN_JSON_APIKEY carries the API key and
	// KOREAN_JSON_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("KOREAN_JSON_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("KOREAN_JSON_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewKoreanJsonSDK(opts)

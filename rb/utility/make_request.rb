...
module KoreanJsonUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = KoreanJsonResponse.new({})
    ...
    if fetch_err
      ...
      response = KoreanJsonResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = KoreanJsonResponse.new(fetched)
    else

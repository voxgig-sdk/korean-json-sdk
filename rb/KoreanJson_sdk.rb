...
require_relative 'KoreanJson_types'
...
class KoreanJsonSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = KoreanJsonHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = KoreanJsonHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, KoreanJsonFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    KoreanJsonUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = KoreanJsonHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = KoreanJsonHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = KoreanJsonHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => KoreanJsonError.new(
        "#{op}_allow",
        "KoreanJsonSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = KoreanJsonHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = KoreanJsonError.new(
        "graphql_error", "KoreanJsonSDK: graphql: #{msg}")
      res["graphql"] = errors
    end

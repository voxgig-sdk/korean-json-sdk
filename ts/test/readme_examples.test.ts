...
//     (\`new KoreanJsonSDK(...)\` / \`KoreanJsonSDK.test(...)\` become
//     \`KoreanJsonSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"comment":{"test01":{"id":"test01"}},"post":{"test01":{"id":"test01"}},"todo":{"test01":{"id":"test01"}},"user":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new KoreanJsonSDK(...)\` and any
// \`KoreanJsonSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+KoreanJsonSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/KoreanJsonSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

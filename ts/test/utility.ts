...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.KOREANJSON_TEST_LIVE ||
    'TRUE' === process.env.KOREANJSON_TEST_OVERRIDE
  ) {
  ...
  m.KOREANJSON_TEST_EXPLAIN = process.env.KOREANJSON_TEST_EXPLAIN || m.KOREANJSON_TEST_EXPLAIN

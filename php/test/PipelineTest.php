...
{
    private static function utility(): KoreanJsonUtility
    {
        return new KoreanJsonUtility();
    }
    ...
    private static function ctx(array $over = []): KoreanJsonContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new KoreanJsonContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new KoreanJsonOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof KoreanJsonError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new KoreanJsonOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = KoreanJsonMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new KoreanJsonOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = KoreanJsonMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new KoreanJsonOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = KoreanJsonMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new KoreanJsonSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = KoreanJsonMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new KoreanJsonResponse([]),
            'result' => new KoreanJsonResult([]),
        ]);
        [, $err] = KoreanJsonMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new KoreanJsonSpec([]),
            'response' => null,
            'result' => new KoreanJsonResult([]),
        ]);
        [, $err] = KoreanJsonMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new KoreanJsonSpec(['step' => 's']),
            'response' => new KoreanJsonResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new KoreanJsonResult([]),
        ]);
        [, $err] = KoreanJsonMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new KoreanJsonSpec(['step' => 's']),
            'response' => new KoreanJsonResponse(self::resp(200, ['v' => 1])),
            'result' => new KoreanJsonResult([]),
        ]);
        [, $err] = KoreanJsonMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new KoreanJsonSpec(['step' => 's']),
            'response' => new KoreanJsonResponse(self::resp(200, ['v' => 2])),
            'result' => new KoreanJsonResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        KoreanJsonMakeResponse::call($ctx);
    ...
    {
        $preset = new KoreanJsonResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new KoreanJsonSpec([]),
            'response' => new KoreanJsonResponse([]),
            'result' => new KoreanJsonResult([]),
        ...
        [$got, $err] = KoreanJsonMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new KoreanJsonResult([])]);
        [, $err] = KoreanJsonMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new KoreanJsonSpec([]), 'result' => null]);
        [, $err] = KoreanJsonMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new KoreanJsonSpec(['step' => 's']),
            'result' => new KoreanJsonResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new KoreanJsonOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = KoreanJsonMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new KoreanJsonSpec(['step' => 's']),
            'result' => new KoreanJsonResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new KoreanJsonOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = KoreanJsonMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new KoreanJsonResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new KoreanJsonSpec([]),
            'result' => new KoreanJsonResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = KoreanJsonMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = KoreanJsonMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new KoreanJsonError('boom', 'boom');
        $utility->fetcher = function (KoreanJsonContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new KoreanJsonSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = KoreanJsonMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (KoreanJsonContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = KoreanJsonMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(KoreanJsonResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        KoreanJsonMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (KoreanJsonContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new KoreanJsonSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = KoreanJsonMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new KoreanJsonResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new KoreanJsonSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = KoreanJsonMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = KoreanJsonMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = KoreanJsonMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new KoreanJsonResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, KoreanJsonDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new KoreanJsonResult(['ok' => false])]);
        $this->expectException(KoreanJsonError::class);
        KoreanJsonDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new KoreanJsonResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', KoreanJsonMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        KoreanJsonMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new KoreanJsonBaseFeature();
        $b = new KoreanJsonBaseFeature();
        KoreanJsonFeatureAdd::call($ctx, $a);
        KoreanJsonFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): KoreanJsonBaseFeature
    {
        $f = new KoreanJsonBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        KoreanJsonFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): KoreanJsonContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new KoreanJsonSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = KoreanJsonPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        KoreanJsonPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new KoreanJsonResponse(['headers' => null]),
            'result' => new KoreanJsonResult([]),
        ]);
        KoreanJsonResultHeaders::call($ctx);

...
class FtCtrl extends KoreanJsonControl
{
...
class FtClient
{
    ...
    private KoreanJsonUtility $utility;

    public function __construct(KoreanJsonUtility $utility, array $options)
    {
    ...
    public function get_utility(): KoreanJsonUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KoreanJsonContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public KoreanJsonContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KoreanJsonContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KoreanJsonConfig::make_feature($name);
    ...
    private static function buildUrl(KoreanJsonSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KoreanJsonError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KoreanJsonResponse($fetched);
        }
    ...
    private function fail(KoreanJsonContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KoreanJsonContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

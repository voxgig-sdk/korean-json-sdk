...
class TodoEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set KOREAN_JSON_TEST_TODO_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function todo_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["todo01", "todo02", "todo03"] as $k) {
    ...
    $entid_env_raw = getenv("KOREAN_JSON_TEST_TODO_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["KOREAN_JSON_TEST_TODO_ENTID"]);

...
class KoreanJsonTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            KoreanJsonTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = KoreanJsonTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = KoreanJsonTestRunner.getenv("KOREAN_JSON_TEST_LIVE")
        override = KoreanJsonTestRunner.getenv("KOREAN_JSON_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = KoreanJsonTestRunner.getenv(key)
        ...
        explain = KoreanJsonTestRunner.getenv("KOREAN_JSON_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if KoreanJsonTestRunner._test_control is not None:
            return KoreanJsonTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                KoreanJsonTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            KoreanJsonTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = KoreanJsonTestRunner.load_test_control()
...
def load_env_local():
    KoreanJsonTestRunner.load_env_local()


def env_override(m):
    return KoreanJsonTestRunner.env_override(m)


def entity_data(v):
    return KoreanJsonTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return KoreanJsonTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return KoreanJsonTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return KoreanJsonTestRunner.load_test_control()


def live_client_options():
    return KoreanJsonTestRunner.live_client_options()


def live_delay_ms():
    return KoreanJsonTestRunner.live_delay_ms()

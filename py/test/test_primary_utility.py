...
from koreanjson_sdk.core.spec import KoreanJsonSpec
from koreanjson_sdk.core.result import KoreanJsonResult
from koreanjson_sdk.core.response import KoreanJsonResponse
from koreanjson_sdk.core.operation import KoreanJsonOperation
from koreanjson_sdk.core.error import KoreanJsonError
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return KoreanJsonError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = KoreanJsonSDK.test(None, None)
        ...
        class TestInitFeature(KoreanJsonBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(KoreanJsonBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = KoreanJsonSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = KoreanJsonSDK.test(None, setup_opts)
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = KoreanJsonSDK.test(None, setup_opts)
